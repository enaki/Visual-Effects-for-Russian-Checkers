...
void initFromFile(){
	FILE *in = fopen("joc.check", "r");
	if(!in){
		glutDestroyWindow(WIN);
		exit(1);
	}

	fscanf(in, "%d %d %d %d %d %d %d %d %d %d", &SIDE_COEF, &MOUSEX, &MOUSEY, &sel.i, &sel.j, &to.i, &to.j, &GO, &POS_MOVES, &HELP); 

	int i, j, check, type;
	for(i = 0; i < R; i++)
		for(j = 0; j < C; j++){
			fscanf(in, "%f %f %d %d", &_board[i][j].x, &_board[i][j].y, &check, &type);	
